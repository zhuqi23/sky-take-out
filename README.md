...
9. controller / serviceImpl 职责:
   - controller: 收前端传来的参数; 简单判断非空(复杂判断反service); 将 service 返回的数据放入 Result; 记录谁在什么时候调用该接口
   - 其他都是 service 的事, 当需要修改时只修改 service
...
1. 规则
   ...
   - 以 redis 为主存储, 进行增删改查
   - 异步批量写入数据库备份: 每隔 5s 批量同步一次 / 使用消息队列削峰填谷, 失败可重试
   - 每次开启项目对比一次 redis 和数据库, 不同以 redis 为准
   - redis 过期时间30天, 数据库90天清理
2. 可能出现的问题
   
   - 提交订单时可能 redis 和数据库不同, 要先做一次强制同步, 以 redis 内容为准
